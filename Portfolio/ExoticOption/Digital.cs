...
namespace ExoticOption
{
    ...
    {
        public Digital(double s, double k, double r, double sigma, double t, int trials, int steps, bool type, bool ant, bool cv, bool mt, double rebate)
          : base(s, k, r, sigma, t, trials, steps, type, ant, cv, mt, rebate, 0, 0)
        {
            S = s;
            K = k;
            Mu = r;
            ...
            if (Ant == true)//choose Ant Var
            {
                ...
                if (CV == true)//choose CV
                {
                    ...
                    for (int i = 0; i < 2 * Sims; i++)
                    {
                        ...
                        if (IsCall == true)
                        {
                            CT[i] = (Rebate * Math.Max((allsims[i, Steps] - K) / Math.Abs(allsims[i, Steps] - K), 0) - cv) * Math.Exp(-Mu * T);
                        }
                        else
                        {
                            CT[i] = (Rebate * Math.Max((K - allsims[i, Steps]) / Math.Abs(K - allsims[i, Steps]), 0) - cv) * Math.Exp(-Mu * T);
                        }
                    }
